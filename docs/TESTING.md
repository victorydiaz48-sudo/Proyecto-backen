...
| `test/telegram.test.ts` | contra un Telegram simulado: sin bot → no disponible y 409; enlace de un solo uso (formato, 30 min, solo el hash en BD), `/start` conecta el chat, responde y audita; código reutilizado, caducado o inventado y mensajes cualesquiera no conectan ni revelan el negocio; enlace nuevo invalida el anterior; desconectar; solo ADMIN; bucle `getUpdates` real y reintento ante errores sin filtrar el token; reserva web → aviso por Telegram en HTML escapado, WhatsApp por su transporte, el panel sin `wa.me` para Telegram; sin chat o cita del panel no hay aviso; envío rechazado → reintento con el error y sin el token |
